...
# check root
[[ $EUID -ne 0 ]] && echo -e "${red} Warning: ${plain} You must use root user to run this script！\n" && exit 1
...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/kruleshvpn/x-ui/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
disable() {
    systemctl disable x-ui
    if [[ $? == 0 ]]; then
        echo -e "${green}x-ui Cancel the boot-up auto-start successfully${plain}"
    else
        echo -e "${red}x-ui Cancel the boot-up failure${plain}"
    fi
...
show_usage() {
     ...
     echo "x-ui disable     -cancel x-ui boot from boot"
